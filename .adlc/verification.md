...
## Gaps
- **Parallel `just e2e` runs collide.** The ports (8100/5180) and the e2e DB path are hard-coded, and `reuseExistingServer: false` makes a second concurrent run fail on the busy port. This matters now, with two implementers in the same working tree. Cheapest fix: read `E2E_API_PORT`/`E2E_WEB_PORT`/`E2E_DB_PATH` from env in `playwright.config.ts`, with today's values as defaults. Until then, serialise e2e runs.
